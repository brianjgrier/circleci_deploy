...
resource "aws_security_group" "circleci_builders_sg" {
  ...
  tags        = merge(var.Tags, {"Name" = format("%s-circleci-builders_sg", var.prefix)})
...
resource "aws_security_group" "circleci_services_sg" {
  ...
  tags        = merge(var.Tags, {"Name" = format("%s-circleci-services_sg", var.prefix)})
  ...
  ingress {
      cidr_blocks = var.aws_nat_eip_list
      protocol = "tcp"
      from_port = 443
      to_port = 443
  }
...
resource "aws_security_group" "circleci_builders_admin_sg" {
  ...
  tags        = merge(var.Tags, {"Name" = format("%s-circleci-builders_admin_sg", var.prefix)})
...
resource "aws_security_group" "circleci_users_sg" {
  ...
  tags        = merge(var.Tags, {"Name" = format("%s-circleci-users_sg", var.prefix)})
  description = "SG representing users of CircleCI Enterprise"

  vpc_id = var.aws_vpc_id

  ingress {
    cidr_blocks = concat(var.Company_IPs, [var.aws_vpc_cidr])
    protocol    = "tcp"
    from_port   = 22
    to_port     = 22
  }
  ...
  ingress {
    cidr_blocks = [var.aws_public_subnet_cidr, var.aws_private_subnet_cidr]
    protocol    = "tcp"
    from_port   = 4647
    to_port     = 4647
  }
...
resource "aws_security_group" "circleci_vm_sg" {
  ...
  tags        = merge(var.Tags, {"Name" = format("%s-circleci-vm_sg", var.prefix)})
...
#  associate_public_ip_address = true
  ...
  tags         = merge(var.Tags, {"Name" = format("%s-circleci-services", var.prefix)})

  volume_tags  = merge(var.Tags, {"Name" = format("%s-circleci-services", var.prefix)}) 
...
module "legacy_builder" {
  ...
  prefix                    = var.prefix
  name                      = "builders"
  aws_subnet_id             = var.aws_private_subnet_id
...
module "nomad" {
  ...
  aws_subnet_cidr_block = var.aws_private_subnet_cidr
  services_private_ip   = aws_instance.services.private_ip
  aws_region            = var.aws_region
  Tags                  = var.Tags 
}
